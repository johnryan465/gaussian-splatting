"""Taken from pytorch3d not using it directly as it is incompatible with the other dependencies"""
...
def se3_exp_map_no_rot(log_transform: torch.Tensor, eps: float = 1e-4) -> torch.Tensor:
    """
    Convert a batch of logarithmic representations of SE(3) matrices `log_transform`
    to a batch of 4x4 SE(3) matrices using the exponential map.
    See e.g. [1], Sec 9.4.2. for more detailed description.
    ...
    T = torch.bmm(V, log_translation[:, :, None])[:, :, 0]
    T = log_translation[None, :, :]

    transform = torch.zeros(
        N, 4, 4, dtype=log_transform.dtype, device=log_transform.device
    )

    transform[:, :3, :3] = torch.eye(3, dtype=log_transform.dtype, device=log_transform.device)[None]
    transform[:, :3, 3] = T
    transform[:, 3, 3] = 1.0
