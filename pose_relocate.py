...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for image_num in range(1, len(viewpoint_stack), 20):
        gaussians = GaussianModel(dataset.sh_degree)
        scene = Scene(dataset, gaussians)
        gaussians.training_setup(opt)
        if checkpoint:
        ...
        offset = None
        viewpoint_stack_idxs_ = [] # = [ i for i in range(len(viewpoint_stack))]
        image_name_str = "0" * (6 - len(str(image_num))) + str(image_num)
        print("Image name", image_name_str)
        tb_writer = prepare_output_and_logger(dataset)



        for i, cam in enumerate(viewpoint_stack):
            random_offset = ((torch.ones(3, device="cuda") * 1) - 0.5) * 0.1 # (torch.rand(3, device="cuda") * 1) - 0.5
            random_offset_tensor = torch.zeros(4, 4, device="cuda")
            # random_offset_tensor[3, :3] = random_offset

            if cam.image_name == image_name_str:
                ...
                # cam.world_view_transform =  torch.inverse(torch.inverse(torch.tensor(random_rotation(2)).float().cuda() @ (cam.world_view_transform) + random_offset_tensor))
            ...
            if True:   
                ...
                with torch.no_grad():
                    ...
                    if iteration == opt.iterations:
                        final_loss[viewpoint_cam.image_name] = ema_loss_for_log
                        print(final_loss)
                        progress_bar.close()

                    # Log and save
                    training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_eval, (pipe, background), viewpoint_stack, viewpoint_cam.image_name)
                    ...
                    if viewpoint_cam.image_name == image_name_str:
                        #print("Original pos", original_pos)
                        # print("Offset", offset)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, train_cameras, current_image_name = None):
    if tb_writer:
    ...
    if iteration in testing_iterations:
        ...
        for _idx, viewpoint in enumerate(train_cameras):
            if viewpoint.image_name == current_image_name:
                idx = _idx
                break

        validation_configs = [{'name': 'train', 'cameras' : [train_cameras[idx]] + [train_cameras[idx % len(train_cameras)] for idx in range(0, 30, 5)] }]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[7_001, 7_050, 7_100, 7_200, 7_300, 7_400, 7_500])
